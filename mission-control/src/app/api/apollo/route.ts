...
export async function POST(request: Request) {
    try {
        ...
        // Fallback to Mock if API Key fails or returns no data
        console.log('Using Fallback Mock Data');
        const mockLeads = Array.from({ length: 5 }).map((_, i) => {
            const lead = {
                id: `mock-${i}`,
                name: `Lead Name ${i + 1}`,
                title: i % 2 === 0 ? 'CTO' : 'Head of Operations',
                company: `${strategy?.industry || 'Tech'} Corp ${i + 1}`,
                email: `contact@company${i + 1}.com`,
                location: strategy?.geo || 'USA',
                employees: strategy?.companySize || '50-200',
                status: 'new' as const,
                score: 80
            };
            return lead;
        ...
        return NextResponse.json({
            success: true,
            leads: mockLeads.sort((a, b) => b.score - a.score)
        });
