...
export async function POST(request: Request) {
    try {
        ...
        console.log(`Triggering Call to ${phone} for ${name}...`);

        // 4. Register Call
        const callResponse = await client.call.createPhoneCall({
            from_number: process.env.RETELL_FROM_NUMBER || "+14155552671", // Must be a Retell-purchased number
            to_number: phone,
        ...
        console.error("Retell Trigger Error:", error);
        return NextResponse.json({
            success: false,
